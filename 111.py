...
def defensive_effort_analysis():
    ...
    st.subheader("Impact of Blocks (BLK) and Steals (STL) on Win Shares (WS)")
    fig = plt.figure(figsize=(10, 8))
    ax = fig.add_subplot(111, projection='3d')

    for metric in selected_metrics:
        ax.scatter(merged_df['BLK'], merged_df['STL'], merged_df['WS'], c=merged_df[metric], label=metric)

    ax.set_xlabel('Blocks (BLK)')
    ax.set_ylabel('Steals (STL)')
    ax.set_zlabel('Win Shares (WS)')
    ax.set_title("LeBron James' Impact Analysis")

    plt.legend()
    st.pyplot(fig)
    st.write("In this graph, we can see that if Lebron have 50 BLK and 180 STL, Then he has the best WS value")
def lebron_can_pass():
